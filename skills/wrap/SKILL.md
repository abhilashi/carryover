---
name: wrap
description: End-of-session debrief for a coding/work session. Auto-captures git state, runs a short structured debrief (goal, blocker, next move, surprises, regression guard, confidence), and writes a persistent handoff to ~/.claude/sessions/<project>/ so the next session can resume with full context. Invoke when finishing or pausing work, or via /wrap.
...
Ask these **one message at a time** (or grouped if the user prefers speed). Reference the auto-captured state so it feels like you were watching. Keep each prompt to one line:
